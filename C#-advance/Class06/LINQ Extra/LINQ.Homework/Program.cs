...
namespace LINQ.Homework
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int songThatStartWithA = Songs.Count(song => song.Name.ToLower().StartsWith("a"));
            Console.WriteLine($"how many Songs start with the letter 'a' (case insensitive) = {songThatStartWithA}. \n===========");


            // - how many artists end with letter 'a' (case insensitive)
            int artistThatEndWithA = Artists.Count(artist => artist.FullName.ToLower().EndsWith("a"));
            Console.WriteLine($"how many artists end with letter 'a' (case insensitive) = {artistThatEndWithA}\n==================");


            // - whats the name of the song with longest duration
            string songWithLongestDuration = Songs.FirstOrDefault(song => song.Duration == Songs.Select(x => x.Duration).Max()).Name;
            Console.WriteLine($"whats the name of the song with longest duration = {songWithLongestDuration}\n=====================");


            // - whats the total Duration of all Songs
            int songsTotalDuration = Songs.Select(song => song.Duration).Sum();
            Console.WriteLine($"whats the total Duration of all Songs = {songsTotalDuration}\n====================================");


            // - how many albums have Songs longer than 300 seconds
            int albumsWithSongLongerThan300Seconds = Songs.Where(song => song.Duration > 300).GroupBy(x => x.AlbumId).Count();
            Console.WriteLine($"how many albums have Songs longer than 300 seconds = {albumsWithSongLongerThan300Seconds}\n=========");


            // - print the names of the artists(separated with "--"), that have more than one album of PopRock genre
            var artistWithMoreThanOnePopRockAlbum = Artists.Where(artist => artist.Albums
            ...
            var albumsOrderedByAverageDuration = Albums.Select(album => new
            {
                Name = album.Name,
                SongDuration = album.Songs.Average(songs => songs.Duration)
            }).OrderByDescending(x => x.SongDuration).ToList();
            Console.WriteLine($"print the name of the album that has highest Average duration of a song = " +
                $" {albumsOrderedByAverageDuration[0].Name}\n============================================");


            // - how many characters has the song that has the shortest Duration
            string songWithShortestDuration = Songs.FirstOrDefault(song => song.Duration == Songs.Min(x => x.Duration)).Name;
            Console.WriteLine($"how many characters has the song that has the shortest Duration =" +
                $" {songWithShortestDuration} and length {songWithShortestDuration.Length} \n=============== ");


            // - print the name and the genre of the album that has most songs
            Album albumWithMostSongs = Albums.FirstOrDefault(album => album.Songs.Count == 
            Albums.Select(x => x.Songs.Count).Max());
            Console.WriteLine($"print the name and the genre of the album that has most songs = " +
                $"Name: {albumWithMostSongs.Name} Genre: {albumWithMostSongs.Genre}\n======================================");


            // - print the name of the artist that has most songs
            var artistWithMostSongs = Artists.Select(artist => new
            {
                Name = artist.FullName,
                NumberOfSongs = artist.Albums.Sum(album => album.Songs.Count)
            }).OrderByDescending(x => x.NumberOfSongs).ToList();
            Console.WriteLine($"print the name of the artist that has most songs = {artistWithMostSongs[0].Name} " +
                $"Songs: {artistWithMostSongs[0].NumberOfSongs}\n=====================");
            //artistWithMostSongs.ForEach(x => Console.WriteLine($"\n{x.Name} -- {x.NumberOfSongs}"));


            // - print the type of the artist(SoloArtist/ Band) that has most albums published before year 2000
            var artistWithMostAlbumsPublishedBeforeYear2000 = Artists.Where(artist => artist.Albums
            .Any(x => x.Year < 2000))
            .Where(artistX => artistX.Albums.Count > 1).ToList();
            Console.WriteLine("print the type of the artist(SoloArtist/ Band) that has most albums published before year 2000");
            artistWithMostAlbumsPublishedBeforeYear2000.ForEach(x =>
            Console.WriteLine($"{x.FullName} type {x.ArtistType} and count: {x.Albums.Count}"));
            Console.WriteLine("============================================");


            // - print the average song duration, of the album that has most songs
            double averageSongDurationOfAlbumWithMostSongs = albumWithMostSongs.Songs.Average(song => song.Duration);
            Console.WriteLine($"print the average song duration, of the album that has most songs = " +
                $"{averageSongDurationOfAlbumWithMostSongs}\n=====================================");


            // Bonus:
            // - print the longest song duration of the album that has least songs
            var albumtWithLeastSongs = Albums.Where(album => album.Songs.Count == Albums.Min(x => x.Songs.Count))
                .Select(albumX => new
                {
                    AlbumName = albumX.Name,
                    SongDuration = albumX.Songs.Max(Song => Song.Duration),
                    NumberOfSongs = albumX.Songs.Count
                }).ToList();
            Console.WriteLine("print the longest song duration of the album that has least songs");
            albumtWithLeastSongs.ForEach(x => Console.WriteLine($"{x.AlbumName} " +
                $"duration: {x.SongDuration} numberOfSongs: {x.NumberOfSongs}"));
            Console.WriteLine("===================================================================");


            // - print the name of the album that has most songs that contain letter 'a' in the name
            var albumWithMostSongsContainingLetterA = Albums.Select(album => new
            {
                AlbumName = album.Name,
                NumberOfSongsThatContainA = album.Songs.Count(song => song.Name.ToLower().Contains("a"))
            }).OrderByDescending(x => x.NumberOfSongsThatContainA).ToList();
            Console.WriteLine($"print the name of the album that has most songs that contain letter 'a' in the name = " +
                $"{albumWithMostSongsContainingLetterA[0].AlbumName} \n============================");
            //albumWithMostSongsContainingLetterA.ForEach(x => Console.WriteLine($"{x.AlbumName} and {x.NumberOfSongsThatContainA}"));


            // - print the name of the artist that has most songs that end with letter 'd'
            var artistWithMostSongThatEndWithD = Artists.Select(artist => new
            {
                ArtistName = artist.FullName,
                NumberOfSongsThatEndWithD = artist.Albums.Sum(album => album.Songs
                .Count(song => song.Name.ToLower().EndsWith("d")))
            }).OrderByDescending(x => x.NumberOfSongsThatEndWithD).ToList();
            Console.WriteLine($"print the name of the artist that has most songs that end with letter 'd'" +
                $" {artistWithMostSongThatEndWithD[0].ArtistName}" +
                $" and number of songs {artistWithMostSongThatEndWithD[0].NumberOfSongsThatEndWithD} \n=============================");
            //artistWithMostSongThatEndWithD.ForEach(x => Console.WriteLine($"{x.ArtistName} and {x.NumberOfSongsThatEndWithD}"));
