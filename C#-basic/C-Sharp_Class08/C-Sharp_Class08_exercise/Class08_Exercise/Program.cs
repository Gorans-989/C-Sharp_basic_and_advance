...
namespace Class08_Exercise
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // if the song . length is longer than 6 add to favorite songs
            foreach (var item in Songs)
            {
                if (item.Length > 6)
                {
                    Maria.FavoriteSongs.Add(item);
                }
            }

            FansArray.Select(x => x.FirstName = "Maria");
            

            Song mariaSongs = (Song)Songs.Where(x => x.Length > 6);
            Maria.FavoriteSongs.Add(mariaSongs);




            // if the song starts with B add to favorite songs
            foreach (var item in Songs)
            {
                if (item.Title.StartsWith("B"))
                {
                    Jerry.FavoriteSongs.Add(item);
                }
            }
